...
class PortfolioQueryEvaluator:
    def __init__(self, model_path: str = OUTPUT_DIR):
        ...
        try:
            # CHANGED: Use setup_model_and_tokenizer directly (no custom model class)
    ...
    def generate_prediction(self, query: str, **generation_kwargs):
        # CHANGED: Format input for Deepseek-coder-1.3b-instruct like in training
...
def main():
    logger.info("Running Deepseek-coder-1.3b-instruct PortfolioQueryEvaluator")
    evaluator = PortfolioQueryEvaluator()

    test_queries = load_test_queries("../ata/test_data.json")
    ...
    # CHANGED: Updated generation parameters for Deepseek-coder-1.3b-instruct
    generation_kwargs = {
        "max_new_tokens": 256,
        "do_sample": True,
        "temperature": 0.3,
