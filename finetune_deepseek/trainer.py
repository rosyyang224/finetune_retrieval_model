...
def train_model():
    logger.info("=" * 60)
    logger.info("PORTFOLIO QUERY Deepseek-coder-1.3b-instruct FINE-TUNING PIPELINE")
    logger.info("=" * 60)
    ...
    wandb.init( 
        ...
        name="Deepseek-coder-1.3b-instruct2-3b_lr9e5_batch4",
        config={
            "model": MODEL_NAME,
            "model_class": "Deepseek-coder-1.3b-instruct",
    ...
    logger.info(f"Creating Deepseek-coder-1.3b-instruct model from scratch: {MODEL_NAME}")
    model = create_model()
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
    
    # IMPORTANT: Set pad token for Deepseek-coder-1.3b-instruct
    ...
    # CHANGED: Skip length grouping for causal LM to avoid tensor issues
